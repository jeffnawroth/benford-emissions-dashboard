...
const { selectedEmissionType, benfordsDistributionData } = storeToRefs(useCountryEmissionStore())
...
const data = computed(() => ({
  ...
  datasets: [
    { data: benfordsDistributionData.value.expected, label: 'Expected', borderWidth: 1 },
    { data: benfordsDistributionData.value.observed, label: 'Observed', borderWidth: 1 },
  ],
}))

const dataSourceCitation = computed(() => selectedEmissionType.value.shortHand === 'CO₂' ? 'Global Carbon Budget (2023) – with major processing by Our World in Data' : 'Jones et al. (2024) – with major processing by Our World in Data')
</script>
